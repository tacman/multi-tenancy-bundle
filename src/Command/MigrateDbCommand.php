...
#[AsCommand(
    ...
    description: 'Migrates databases for registered tenants'
)]
final class MigrateDbCommand extends Command
{
    ...
    protected function interact(InputInterface $input, OutputInterface $output): void
    {
        ...
        $this->io->text([
            ...
            '',
            ' $ php bin/console tenant:migrations:migrate email@example.com',
            '',
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $stopwatch->start('migrate-tenant-database-command');
        ...
        if(!$tenant){
            ...
            try{
                ...
                $otherCommand = new \Doctrine\Migrations\Tools\Console\Command\MigrateCommand($this->getDependencyFactory($input, $dbName));
                $otherCommand->run($newInput, $output);

                // set migration status 
                $tenant->setMigrationStatus(MigrationStatusEnum::MIGRATION_CREATED);
                $connection->changeDatabase($this->mainDb);
                $this->em->persist($tenant);
                $this->em->flush();

                $this->io->success(sprintf('%s database was successfully migrated (%s): %s (%s)', 'Tenant', $dbName, $tenant->getEmail(), $tenant->getName()));
                $newInput = new ArrayInput([
                    //'version' => 'latest'
                ]);
                $newInput->setInteractive(false);
        ...
        $event = $stopwatch->stop('migrate-tenant-database-command');
        if ($output->isVerbose()) {
    ...
    protected function getDependencyFactory(InputInterface $input, string $dbName): DependencyFactory
    {
        ...
        $tenantMigrationConfig = new ConfigurationArray(
            //$this->parameterBag->get('doctrine_migrations')
            ["migrations_paths" => ['DoctrineMigrations\Main'=> $this->projectDir . '/migrations/main']]
        );
